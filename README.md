...
| `CHIKIN_EGRESS_SUBNET` | `172.28.0.0/16` | Subnet for the egress network the fleet shares. Pinned so a host-firewall rule that names it (`bin/chikin-allow-host`) is not stranded when the network is recreated. Substituted into `docker-compose.yml`, so it belongs in `.env` and is never gateway env. Adopting or changing it recreates the egress network and every container on it — even at the value Docker already assigned — and is not a plain `up -d` over a live fleet: gateway-provisioned `chikin-chrome-*` containers carry no compose labels, so compose stops the gateway and then cannot remove the network they are still attached to, leaving the stack down. Bring the fleet down first, browsers included: `docker rm -f $(docker ps -aq --filter label=chikin.fleet=1)` (profile volumes are untouched), `make down`, then `make up`. Make sure the range is free too: on a collision, `up` removes the old network and then fails to create the new one ("Pool overlaps with other one on this address space"), leaving no egress network. |
...
1. **Nav watchdog** — after the child reports a navigation succeeded, the gateway compares the page the *child* reports it is bound to against the container's CDP `/json/list` (ground truth), sampled both at reply time and after a settle delay. Two consecutive navs on which the child is provably bound to a page the browser does not have force a transparent child respawn, which re-binds the browser's real current target. The test is deliberately not "did the page set move?" — a nav that redirects onto the page you are already on moves nothing while the child is perfectly healthy, and a page that redirects on its own after the reply is still cleared by the reply-time sample. Repeated CDP connection failures on the child's stderr (e.g. the container was removed out-of-band) trigger the same respawn. The child's view is read from the human-readable `## Pages` block in its reply, which is the **only** channel that reaches the gateway *under the flags it spawns children with*: the child also builds a machine-readable `structuredContent.pages`, but it copies that onto a tool result only when `--experimentalStructuredContent` is passed, and the `chrome-devtools-mcp` binary leaves that flag off — the same in 1.1.1 and 1.6.0, so a version bump does not change it (#75). Only the package's sibling `chrome-devtools` CLI puts that flag in its defaults. The MCP SDK is not what drops it: it validates `structuredContent` against an `outputSchema` when a tool declares one, and never removes the field. So a prose format is load-bearing by default, and it broke — 1.6.0 renders a titled page as `<id>: <title> (<url>)` instead of `<id>: <url>`, and a parse that assumed the latter selected nothing, so the watchdog went blind with every test still green. Setting `CDM_EXTRA_ARGS=--experimentalStructuredContent` is a supported way to put the structured channel back on the wire, and the gateway prefers it whenever it is present — which is why that code path is kept rather than deleted. `itest/cdm-wire.mjs` proves a real reply still parses to a selected page; `gateway/test/cdm-outputschema.test.ts` is the CI-side tripwire on upstream declaring a schema.
...
- **A browser cannot reach a service on this host** where the host firewall denies inbound by default — Omarchy ships ufw enabled with `DEFAULT_INPUT_POLICY="DROP"`. Container→host traffic is INPUT on the host, and Docker installs nothing there, so the host's default policy has the last word; published-port ingress, container→internet egress and embedded DNS ride FORWARD/NAT through Docker's own chains and are unaffected, as is `enable_icc=false` on the egress network. Every host address a browser might try is dropped alike: the bridge gateway, `docker0`, the LAN address, the tailnet address. Binding the server to `0.0.0.0` does not help. Read the other way this is the posture working — a compromised browser cannot reach host services — so open one port at a time with `bin/chikin-allow-host <port>`, never a blanket `ufw allow from <subnet>`, which hands every service on the machine, sshd included, to the least-trusted process on it. The gateway probes the path at startup and reports it in `/healthz`'s `warnings`.
...
**A browser can't open a dev server running on this host** (`Navigation timeout of 10000 ms exceeded`, at every address you try). The host's firewall is dropping it, not the server. A container reaching its own host is INPUT traffic, which Docker leaves to the host's default policy — and on a default-deny host (ufw with `DEFAULT_INPUT_POLICY="DROP"`, Omarchy's default) the bridge gateway, `docker0`, the LAN address and the tailnet address all fail the same way, whatever the server is bound to. The container's own `127.0.0.1` refuses instantly instead, because inside the container that is the container. Allow the one port: `bin/chikin-allow-host 5173`, then reach the host at `host.docker.internal` (or the address that command prints). Once the port is allowed the bind address starts to matter: the server must listen on a non-loopback address (`--host 0.0.0.0` for Vite and Next, which default to `localhost`), or the browser now gets an instant `ERR_CONNECTION_REFUSED` in place of the timeout. `bin/chikin-allow-host --status` lists what is already open. The gateway probes this at startup, so `/healthz`'s `warnings` says so up front — but the probe reads the host's default policy, not per-port rules, so that warning stays after you allow a port; `--status` is the record. See [Security](#security).
