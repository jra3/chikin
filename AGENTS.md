...
- **A new gateway env knob needs a line in `docker-compose.yml` too, or `.env` is inert.** Compose passes only the vars listed under the gateway service's `environment:`; `config.ts` reading one is not enough. `CHIKIN_VOLUME_GC` is documented in `README.md` and `.env.example` but missing from compose, so setting it there does nothing today. Pairs with the frozen-env note above: `/healthz` reports what the process actually has.
